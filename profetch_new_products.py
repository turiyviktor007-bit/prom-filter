...
# Довідник груп Пром: Номер_групи → (Назва_групи, Адреса_підрозділу, Ідентифікатор_підрозділу)
PROM_GROUP_MAP = {
...
# Маппінг категорій фіду (з сайту Profetch) → групи Пром
# Нові категорії сайту, яких нема прямо у Пром, прив'язуються до відповідної групи Пром
FEED_CATEGORY_MAP = {
    "154816597": "128172640",  # Автодержатели → (тимчасово стекла, налаштуйте за потреби)
    "154816598": "128172640",  # Зарядные устройства
    "154816599": "128172640",  # Защитные стекла → Стекла защитные
    "154816600": "128172640",  # Защитные пленки → Стекла защитные
    "154816602": "116950860",  # Чехлы для планшетов → Чехлы
    "154816603": "45546",      # Сумки/рюкзаки → Корневая
    "154816604": "131240127",  # Наушники → Наушники и гарнитуры
    "154816605": "131240127",  # Переходники/адаптеры → Наушники и гарнитуры
}

# Параметри фіду які треба винести в окремі колонки (не в характеристики)
DIMENSION_PARAMS = {
    "Вес в упаковке, кг": "weight",
    "Вага в пакованні, кг": "weight",
    "Ширина в упаковке (см)": "width",
    "Ширина в пакованні (см)": "width",
    "Высота в упаковке (см)": "height",
    "Висота в пакованні (см)": "height",
    "Глубина в упаковке (см)": "length",
    "Глибина в пакованні (см)": "length",
}
...
def parse_feed(source, on_progress):
    ...
    for o in offers:
        ...
        gtin = ""
        mpn = ""
        dims = {"weight": "", "width": "", "height": "", "length": ""}
        ...
        products.append({
            ...
            "pictures": pictures, "params": params,
            "gtin": gtin, "mpn": mpn, "dims": dims,
        })
    return products
...
def generate_xls(new_items, output_path, on_progress):
    ...
    for idx, p in enumerate(new_items):
        ...
        d = p["dims"]
        row = [
            p["vendor_code"],          # 1 Код_товара
            p["name"],                 # 2 Название_позиции
            "",                        # 3 Название_позиции_укр
            "", "",                    # 4-5 Поисковые запросы
            p["description"],          # 6 Описание
            "",                        # 7 Описание_укр
            "r",                       # 8 Тип_товара
            p["price"],                # 9 Цена
            p["currency"] or "UAH",    # 10 Валюта
            "шт.",                     # 11 Единица_измерения
            "", "", "",                # 12-14 мин объём, опт цена, мин опт
            ", ".join(p["pictures"]),  # 15 Ссылка_изображения
            avail,                     # 16 Наличие
            "",                        # 17 Количество
            group_num,                 # 18 Номер_группы
            grp_name,                  # 19 Название_группы
            grp_addr,                  # 20 Адрес_подраздела
            "", "", "", "",            # 21-24 поставка, упаковка
            "",                        # 25 Уникальный_идентификатор
            p["id"],                   # 26 Идентификатор_товара (внешний ID)
            grp_subid,                 # 27 Идентификатор_подраздела
            "",                        # 28 Идентификатор_группы (пусто)
            p["vendor"],               # 29 Производитель
            p["country"],              # 30 Страна_производитель
            "", "", "", "",            # 31-34 скидка, группа разнов, заметки, на сайте
            "", "", "",                # 35-37 скидки от/до, цена от
            "",                        # 38 Ярлык (пусто)
            "", "", "", "",            # 39-42 HTML заголовок/описание
            p["gtin"],                 # 43 GTIN
            p["mpn"],                  # 44 MPN
            d["weight"],               # 45 Вес,кг
            d["width"],                # 46 Ширина,см
            d["height"],               # 47 Высота,см
            d["length"],               # 48 Длина,см
